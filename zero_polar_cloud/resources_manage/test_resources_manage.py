...
class Test_resources_manage:
    ...
    @pytest.mark.resources_manage
    def test_computer_manage(self, caseinfo):
        RequestsUtil(DebugTalk()).standard_yaml_testcase(caseinfo)

    @pytest.mark.parametrize("caseinfo", read_testcase_yaml("/zero_polar_cloud/resources_manage/cabinet_manage.yaml"))
    @allure.story("机柜管理")
    @allure.title("{caseinfo[case_name]}")
    @pytest.mark.cabinet_manage
    def test_cabinet_manage(self, caseinfo):
        RequestsUtil(DebugTalk()).standard_yaml_testcase(caseinfo)
